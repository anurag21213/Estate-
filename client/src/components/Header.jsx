    ...
    return (
        <header className='flex bg-slate-200 shadow-md  items-center justify-center'>
            <div className='flex justify-between w-full sm:w-[80%] items-center p-3' >
                <Link to='/'>
                <h1 className=' title flex flex-wrap text-sm sm:text-xl font-bold'>
                ...
                <input type='text' placeholder='Search...' className='search bg-transparent  border-none w-24 sm:w-64  focus:outline-none  ' value={searchTerm} onChange={(e)=>{setSearchTerm(e.target.value)}} />
                <button>
                ...
                <Link to='/profile'>{currUser?<img src={currUser.avatar} alt='profile' className='profile h-7 w-7 rounded-full object-cover '/>:<li className=' text-slate-700 hover:underline cursor-pointer'>Sign up</li>}</Link>
                </ul>
            </div>
